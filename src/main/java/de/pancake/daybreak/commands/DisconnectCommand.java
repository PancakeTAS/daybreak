    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        if (sender instanceof Player p) {
            ...
            if (combat.timers.containsKey(p)) {
                p.sendMessage(miniMessage().deserialize("<prefix><red>You are currently in combat.</red>", PREFIX));
                return true;
            }

            if (combat.disconnects.containsKey(p)) {
                p.sendMessage(miniMessage().deserialize("<prefix><red>You are already disconnecting.</red>", PREFIX));
                return true;
            }

            combat.disconnects.put(p, 5);
            p.sendMessage(miniMessage().deserialize("<prefix><green>You are now disconnecting. Do not log off. Do not move. You will be kicked in 5 seconds</green>", PREFIX));
        }
        return true;
    }
